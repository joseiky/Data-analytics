...
# ===== 1. Load and preprocess Dataset.xlsx =====
...
# ===== 2. Load and decode variant data =====
...
df_variant["Age Group"] = pd.cut(df_variant["AGE"], bins=[0, 9, 19, 29, 39, 49, 59, 69, 79, 200],
                                 labels=['0-9', '10-19', '20-29', '30-39', '40-49',
                                         '50-59', '60-69', '70-79', '80+'])
...
def plot_agewise(ax, data, x, hue, rotation=90, top_n=None):
    df_plot = data[[x, hue]].dropna()
    if top_n:
    ...
    if df_plot.empty:
        ax.axis("off")
        return
    sns.countplot(data=df_plot, x=x, hue=hue, ax=ax, order=df_plot[x].value_counts().index)
    ax.tick_params(axis='x', rotation=rotation)
    for container in ax.containers:
        ax.bar_label(container, fontsize=6, padding=2)
    p = chi2_p(df_plot[x], df_plot[hue])
    ax.set_title(f"{x} vs {hue} ({p_to_stars(p)})", fontsize=10)
    ax.set_xlabel("")
    ax.set_ylabel("Count")
    ax.legend(loc='best', fontsize=6)

# ===== 5. Variables to plot =====
demo_vars = ["Source", "Specimen", "Pt Gender", "Ethnicity", "Result", "Abnormal"]
variant_vars = ["Variant_decoded", "mRNA_decoded", "Protein_decoded", "Common Name_decoded"]

# ===== 6. Figure 5A: Age Group on x-axis =====
fig5a, axs5a = plt.subplots(4, 3, figsize=(18, 22))
axs5a = axs5a.flatten()

for i, var in enumerate(demo_vars):
    plot_agewise(axs5a[i], df, "Age Group", var)

for j, var in enumerate(variant_vars):
    plot_agewise(axs5a[len(demo_vars) + j], df_variant, "Age Group", var, top_n=10)

for k in range(len(demo_vars) + len(variant_vars), len(axs5a)):
    axs5a[k].axis("off")

plt.tight_layout()
plt.savefig("Figure_5A_AgeGroup_on_X.png", dpi=300)
plt.savefig("Figure_5A_AgeGroup_on_X.tiff", dpi=300)
plt.close()

# ===== 7. Figure 5B: Age Group as hue =====
fig5b, axs5b = plt.subplots(4, 3, figsize=(18, 22))
axs5b = axs5b.flatten()

for i, var in enumerate(demo_vars):
    plot_agewise(axs5b[i], df, var, "Age Group")

for j, var in enumerate(variant_vars):
    plot_agewise(axs5b[len(demo_vars) + j], df_variant, var, "Age Group", top_n=10)

for k in range(len(demo_vars) + len(variant_vars), len(axs5b)):
    axs5b[k].axis("off")

plt.tight_layout()
plt.savefig("Figure_5B_AgeGroup_as_Hue.png", dpi=300)
plt.savefig("Figure_5B_AgeGroup_as_Hue.tiff", dpi=300)
plt.close()
